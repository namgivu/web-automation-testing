...
# add my folder to PYTHONPATH ref. http://flask.pocoo.org/docs/0.11/deploying/mod_wsgi/#creating-a-wsgi-file
s=os.path.abspath(os.path.dirname(__file__)) ; APP_HOME=os.path.abspath('%s/..' % s)
...
def takeSnapshot(driver, prefix=None, suffix=None, forceSnapshot=False, printOutcome=False):
    ...
    if printOutcome:  # only print snapshot when required
        from .logger import l; l(f'Snapshot taken at {filename}')
